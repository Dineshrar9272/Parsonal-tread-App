...
# ============================================================================
# PROTOCOL 2: CRASH-PROOF CSS INJECTION (NO F-STRING STRING INTERPOLATION)
# ============================================================================
# Standard string addition used instead of f-string to bypass bracket execution crashes completely
st.markdown("<style>" + 
...
with col_search_frame:
    user_symbol_selection_input = st.text_input(
        ...
        value="BTC",
        placeholder="Enter asset asset key...",
        label_visibility="collapsed"
    )
    resolved_active_symbol = user_symbol_selection_input.upper().strip()

with col_ema_flag:
    interface_checkbox_ema_ribbon = st.checkbox("Overlay EMAs (12/26)", value=True)
with col_vol_flag:
    interface_checkbox_vol_profile = st.checkbox("Volume Indicators", value=True)
...
if secondary_workspace_right_grid is not None:
    with secondary_workspace_right_grid:
        st.markdown('<div class="terminal-panel-frame" style="height: ' + str(dynamic_calculated_chart_pixel_height) + 'px; overflow-y: auto; margin-bottom: 0px;">' +
        ...
        if active_focus_market_matrix['last_price'] >= active_focus_market_matrix['weighted_avg_price_vwap']:
            ...
            st.markdown('<p style="font-size:11px; color:#848e9c; line-height:1.4;">Asset trading systematically above primary VWAP boundaries. Institutional buy pressure tracking stable.</p>', unsafe_html=True)
        else:
            st.markdown('<div style="margin-bottom:8px;"><span class="tag-bearish-indicator">STRUCTURE BEARISH</span></div>', unsafe_html=True)
            st.markdown('<p style="font-size:11px; color:#848e9c; line-height:1.4;">Spot valuation printing under mean session profiles. Supply side pressure dominant over short horizons.</p>', unsafe_html=True)
...
if not st.session_state.fullscreen_mode:
    ...
    # ADVANCED TECHNICALS SUB-MODULE
    if st.session_state.terminal_active_tab == "📈 Advanced Algorithmic Calculations Engine":
        ...
        with math_ui_col1:
            st.metric(
                ...
                delta="Crossover Stable" if latest_computed_index_row['EMA_Fast'] > latest_computed_index_row['EMA_Slow'] else "Compression Warning"
            )
        with math_ui_col2:
            st.metric(
    ...
    # RISK ALLOCATION ENGINE SUB-MODULE
        ...
        with allocation_layout_left_col:
            ...
            if absolute_invalidation_distance_spread > 0:
                ...
                st.markdown(
                    '<div style="background-color:#0b0d12; border:1px dashed #2d323f; padding:12px; border-radius:4px; font-family:monospace; font-size:12px; margin-top:10px;">'
                    '<span style="color:#848e9c;">CAPITAL POOL AT MAXIMUM RISK  :</span> <span style="color:#f6465d; font-weight:bold;">$' + f"{capital_value_exposure_risk_pool:,.2f}" + '</span><br>'
                    '<span style="color:#848e9c;">OPTIMIZED CONTRACT UNIT SIZE   :</span> <span style="color:#0ecb81; font-weight:bold;">' + f"{optimized_unit_contract_capacity:.4f}" + ' ' + resolved_active_symbol + '</span><br>'
                    '<span style="color:#848e9c;">NOTIONAL VALUE VALUE BRACKET  :</span> <span style="color:#ffffff; font-weight:bold;">$' + f"{total_notional_leverage_volume:,.2f}" + '</span><br>'
                    '<span style="color:#848e9c;">IMPLIED CALCULATED LEVERAGE RATIO:</span> <span style="color:#7047eb; font-weight:bold;">' + f"{mathematical_implied_leverage_ratio:.2f}" + 'x Leverage</span>'
                    '</div>',
                    unsafe_html=True
                )
    ...
    # COGNITIVE QUANT ENVIRONMENT SUB-MODULE
        ...
        st.markdown('<div class="terminal-panel-frame">' +
                    ...
                    '<span style="color:#7047eb; font-weight:bold; font-size:12px; font-family:monospace;">💬 CORE SYSTEM INTERACTION INTERFACE</span>' +
                    '<span class="tag-bullish-indicator">AGENT: LIVE</span>' +
                    '</div>' +
                    '<p style="font-size:12px; color:#b7bdc6; line-height:1.5; margin:0px;">Quant tracking logic actively evaluating telemetry targets for <b>' + resolved_active_symbol + 'USDT</b>. Core algorithmic engine synchronized. Submit structural queries below to extract prompt models updates.</p>' +
        ...
        if user_ai_prompt_entry:
            ...
            st.markdown('<div style="background-color:#020304; border:1px solid #7047eb; padding:12px; border-radius:4px; font-family:monospace; font-size:12px; color:#e2e4e9; margin-top:8px;">' +
                        '<b style="color:#7047eb;">🤖 QUANT ENGINE COMPUTATION FEED RESPONSE:</b><br>' +
                        f"Processed strategy context parameters for asset identifier {resolved_active_symbol} at terminal index level ${active_focus_market_matrix['last_price']:,.2f}. Mathematical cross-confluences support short-term standard distribution constraints. Risk limits remain within default levels." +
                        '</div>', unsafe_html=True)

    # DEFAULT FALLBACK GRID INTERFACE MODULE
    else:
        ...
        with dashboard_row_split_left:
            st.markdown('<div class="terminal-panel-frame" style="height:210px;">' +
                        ...
                        '<p style="font-size:11.5px; line-height:1.4; color:#848e9c;">Calculated data arrays confirm standard liquidity channels match standard performance models. Cross-reference metrics across alternative tabs to check indicator confluences.</p>' +
                        '</div>', unsafe_html=True)
        with dashboard_row_split_right:
            st.markdown('<div class="terminal-title-bar" style="margin-bottom:4px;">⏱️ CORE RUNTIME TERMINAL SYSTEM ACTIVITY EVENTS LOGS</div>', unsafe_html=True)
