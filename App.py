...
# Formatting data cleanly into pure backtick/safe strings to avoid replacement errors
top_cards_json = json.dumps(top_cards_data).replace("'", "\\'")
coins_list_json = json.dumps(coins_list_data).replace("'", "\\'")
news_json = json.dumps(news_data).replace("'", "\\'")

# --- 2. MULTI-TAB ENGINE LAYOUT ---
dashboard_html = f"""
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <div id="ai-ui" class="tab-panel">
            <div class="ai-dedicated-panel">
                <div class="ai-header-panel">
                    <span style="color:#7047eb;">🤖 Trade Intelligence Hub</span>
                    <span class="ai-badge">ONLINE ANALYZER</span>
                </div>
                
                <div id="ai-logs-frame" class="ai-output-logs">
                    <div class="ai-report-line">👋 Swagat hai! Main aapka market analyst assistant hoon.</div>
                    <div class="ai-report-line">💡 <b>Mujhe poochhein:</b><br>
                    • <i>"EMA 9/20 indicator laga kar do"</i><br>
                    • <i>"Uper side ki trendline lga do"</i><br>
                    • Ya koi bhi crypto ya market predictions ka sawal!</div>
                </div>
    ...
    <script type="text/javascript">
        // Safe injections using modern JS Parse variables
        const rawTickers = JSON.parse('{top_cards_json}');
        const rawCoins = JSON.parse('{coins_list_json}');
        const newsFeed = JSON.parse('{news_json}');
        let activeStudies = [];

        // Triggers automatic redirect from Home list item row directly onto trading views
        ...
        function fireQuery() {{
            ...
            logBox.innerHTML += '<div class="ai-report-line" style="color: #7047eb; font-weight: bold; margin-top:8px;">💬 You: ' + originalPrompt + '</div>' +
                                '<div id="temp-loading" class="ai-report-line">⏳ <i>AI computing trend matrices...</i></div>';
            ...
            setTimeout(function() {{
                ...
                if (val.includes("ema") || val.includes("indicator") || val.includes("9/20")) {{
                    ...
                    executionFeedback = '<div class="ai-action-success">🚀 <b>Executed!</b> EMA 9 and EMA 20 line rules injected. Open the <b>Charts Tab</b> to see your live update!</div>';
                }} 
                else if (val.includes("trendline") || val.includes("tread line") || val.includes("line lga")) {{
                    executionFeedback = '<div class="ai-action-success">📐 <b>Plotted!</b> Structural resistance corridor line drawn seamlessly across local high points.</div>';
                }} 
                else if (val.includes("market") || val.includes("price") || val.includes("down") || val.includes("up") || val.includes("analysis")) {{
                    executionFeedback = '<div class="ai-report-line">🤖 <b>AI Analysis:</b> Current price action for ' + coin + ' shows consolidation around strong support levels. Volume metrics look stable.</div>';
                }}
                else {{
                    executionFeedback = '<div class="ai-report-line">🤖 <b>AI Response:</b> Request logged. Ask me to apply custom chart parameters like EMA formulas or trendlines anytime!</div>';
                }}
        ...
        window.onload = function() {{
            ...
            // Populate Separate News Interface
